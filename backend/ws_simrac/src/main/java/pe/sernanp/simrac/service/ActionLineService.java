...
@Service
public class ActionLineService {
	...
	public ResponseEntity save (ActionLineModel item) throws Exception {
		try {
			...
			if (id == 0) {
				ActionLineModel item2 = this._repository.save(item);
				id = item2.getId();
	...
	public ResponseEntity<ActionLineModel> list() throws Exception{
		try {
			ResponseEntity<ActionLineModel> response = new ResponseEntity<ActionLineModel>();
			List<ActionLineModel> items = _repository.findAll();
	...
	public ResponseEntity<ActionLineModel> detail(int id) throws Exception {
		try {
			if (id == 0) {
			...
			ResponseEntity<ActionLineModel> response = new ResponseEntity<ActionLineModel>();
			ActionLineModel item = _repository.findById(id).get();
