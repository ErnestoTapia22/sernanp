...
public class AlliedCommitmentService extends BaseService<AlliedCommitmentModel> {
	...
	@Transactional
	public ResponseEntity save(AlliedCommitmentModel item) throws Exception {		
	...
	public ResponseEntity<AlliedCommitmentModel> detail(int id) throws Exception {
		try {
			if (id == 0) {
				throw new Exception("No existe el elemento");
			}
			boolean success = true;
			ResponseEntity<AlliedCommitmentModel> response = new ResponseEntity<AlliedCommitmentModel>();
			AlliedCommitmentModel item = this._repository.detail(this._dataSource, id);
