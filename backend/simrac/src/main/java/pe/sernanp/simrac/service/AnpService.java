...
public class AnpService extends BaseService<AnpModel> {
	...
	@Transactional
	public ResponseEntity save(AnpModel item) throws Exception {		
	...
	public ResponseEntity<AnpModel> detail(int id) throws Exception {
		try {
			if (id == 0) {
			...
			ResponseEntity<AnpModel> response = new ResponseEntity<AnpModel>();
			AnpModel item = this._repository.detail(this._dataSource, id);
