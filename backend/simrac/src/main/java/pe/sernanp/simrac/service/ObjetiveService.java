...
@Service
public class ObjetiveService extends BaseService<ObjetiveModel> {
	...
	public ResponseEntity<ObjetiveModel> detail(int id) throws Exception {
		try {
			...
			ObjetiveModel item = this._repository.detail(this._dataSource, id);
	...
	public ResponseEntity<ActionLineModel> searchActionLines(int id) throws Exception {
		try {
			...
			List<ActionLineModel> item = this._repository.searchActionLines(this._dataSource, id);
			response.setSuccess(success);
